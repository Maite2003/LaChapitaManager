...
from ui.variant_dialog import VariantDialog
...
class AddProductDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        edit_variant_btn = QPushButton("Editar variante")
        edit_variant_btn.clicked.connect(self.edit_variant)
        delete_variant_btn = QPushButton("Eliminar variante")
        delete_variant_btn.clicked.connect(self.delete_variant)

        variant_btn_layout.addWidget(add_variant_btn)
        variant_btn_layout.addWidget(edit_variant_btn)
        variant_btn_layout.addWidget(delete_variant_btn)
    ...
    def delete_variant(self):
        ...
        if selected < 0 or selected >= len(self.variants):
            ...
            return
        del self.variants[selected]
        self.refresh_variant_table()

    def load_variants(self):
        """Carga las variantes del producto si existe, o inicializa una lista vacía."""
        self.variants = Product.get_product_with_variants(self.product)["variants"] if self.product else []
        self.refresh_variant_table()
